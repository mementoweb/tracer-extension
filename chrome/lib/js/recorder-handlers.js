        ...
        // END
            ...
            if ('input' == tagName && Recorder.inputTypes.indexOf(type) >= 0) {
                if (event.target.value.length > 0) {
                    this.record("type", this.locatorBuilders.buildAll(event.target), event.target.value, baseURI);
                    ...
                    if (enterTarget != null) {
                        ...
                        if (formChk == 'form' && (tempTarget.hasAttribute("id") || tempTarget.hasAttribute("name")) && (!tempTarget.hasAttribute("onsubmit"))) {
                            ...
                            this.record("sendKeys", this.locatorBuilders.buildAll(enterTarget), "${KEY_ENTER}", baseURI);
                        enterTarget = null;
                    }
                    // END
                } else {
...
Recorder.addEventHandler('doubleClickAt', 'dblclick', function(event) {
    ...
    this.record("doubleClickAt", this.locatorBuilders.buildAll(event.target), left + ',' + top, baseURI);
}, true);
// END
        ...
        if (tagName == 'input' && Recorder.inputTypes.indexOf(type) >= 0) {
            if (key == 13) {
                ...
                if (tempValue == enterTarget.value && tabCheck == enterTarget) {
                    ...
                    if (formChk == 'form' && (tempTarget.hasAttribute("id") || tempTarget.hasAttribute("name")) && (!tempTarget.hasAttribute("onsubmit"))) {
                        if (tempTarget.hasAttribute("id"))
                            this.record("submit", [["id=" + tempTarget.id]], "", baseURI);
                        else if (tempTarget.hasAttribute("name"))
                            this.record("submit", [["name=" + tempTarget.name]], "", baseURI);
                    } else
                        this.record("sendKeys", this.locatorBuilders.buildAll(enterTarget), "${KEY_ENTER}", baseURI);
                    enterTarget = null;
                }
                    ...
                    // END
                        ...
                        if ('input' == tagName && Recorder.inputTypes.indexOf(type) >= 0) {
                            if (typeTarget.value.length > 0) {
                                this.record("type", this.locatorBuilders.buildAll(typeTarget), typeTarget.value, baseURI);
            ...
            if ((key == 38 || key == 40) && event.target.value != '') {
                ...
                if (tempbool) {
                    this.record("type", this.locatorBuilders.buildAll(event.target), tempValue, baseURI);
                }
                ...
                if (key == 38) this.record("sendKeys", this.locatorBuilders.buildAll(event.target), "${KEY_UP}", baseURI);
                else this.record("sendKeys", this.locatorBuilders.buildAll(event.target), "${KEY_DOWN}", baseURI);
                tabCheck = event.target;
            }
            if (key == 9) {
                if (tabCheck == event.target) {
                    this.record("sendKeys", this.locatorBuilders.buildAll(event.target), "${KEY_TAB}", baseURI);
                    preventType = true;
                }
            }
        }
    ...
    if (this.selectMousedown) {
        ...
        if (this.selectMousedown && event.button === 0 && (x + y) && (event.clientX < window.document.documentElement.clientWidth && event.clientY < window.document.documentElement.clientHeight) && getSelectionText() === '') {
            ...
            if (!!this.mouseoverQ.length && this.mouseoverQ[1].relatedTarget == this.mouseoverQ[0].target && this.mouseoverQ[0].target == event.target) {
                ...
                this.record("mouseDownAt", this.locatorBuilders.buildAll(this.selectMousedown.target), sourceRelateX + ',' + sourceRelateY, baseURI);
                this.record("mouseMoveAt", this.locatorBuilders.buildAll(this.mouseoverQ[1].target), targetRelateX + ',' + targetRelateY, baseURI);
                this.record("mouseUpAt", this.locatorBuilders.buildAll(this.mouseoverQ[1].target), targetRelateX + ',' + targetRelateY, baseURI);
            } else {
                ...
                this.record("mouseDownAt", this.locatorBuilders.buildAll(event.target), targetRelateX + ',' + targetRelateY, baseURI);
                this.record("mouseMoveAt", this.locatorBuilders.buildAll(event.target), targetRelateX + ',' + targetRelateY, baseURI);
                this.record("mouseUpAt", this.locatorBuilders.buildAll(event.target), targetRelateX + ',' + targetRelateY, baseURI);
            }
        }
    } else {
        ...
        if (this.mousedown && this.mousedown.target !== event.target && !(x + y)) {
            this.record("mouseDown", this.locatorBuilders.buildAll(this.mousedown.target), '', baseURI);
            this.record("mouseUp", this.locatorBuilders.buildAll(event.target), '', baseURI);
...
Recorder.addEventHandler('dragAndDropToObject', 'drop', function(event) {
    ...
    if (this.dragstartLocator && event.button == 0 && this.dragstartLocator.target !== event.target) {
        //value no option
        var baseURI = event.target.baseURI;
        this.record("dragAndDropToObject", this.locatorBuilders.buildAll(this.dragstartLocator.target), this.locatorBuilders.build(event.target), baseURI);
    }
...
Recorder.addEventHandler('mouseOut', 'mouseout', function(event) {
    if (this.mouseoutLocator !== null && event.target === this.mouseoutLocator) {
        var baseURI = event.target.baseURI;
        this.record("mouseOut", this.locatorBuilders.buildAll(event.target), '', baseURI);
    }
...
Recorder.addEventHandler('mouseOver', 'DOMNodeInserted', function(event) {
    if (pageLoaded === true && window.document.documentElement.getElementsByTagName('*').length > nowNode) {
        ...
        if (this.nodeInsertedLocator) {
            this.record("mouseOver", this.locatorBuilders.buildAll(this.nodeInsertedLocator), '', baseURI);
    ...
    if (checkFocus == 1) {
        if (event.target == getEle) {
            if (getEle.innerHTML != contentTest) {
                var baseURI = event.target.baseURI;
                this.record("editContent", this.locatorBuilders.buildAll(event.target), getEle.innerHTML, baseURI);
            }
            checkFocus = 0;
        }
    }
    ...
    if (event.target.tagName) {
        ...
        if ('select' == tagName) {
            if (!event.target.multiple) {
                ...
                this.record("select", this.locatorBuilders.buildAll(event.target), this.getOptionLocator(option), baseURI);
            } else {
                var options = event.target.options;
                for (var i = 0; i < options.length; i++) {
                    if (options[i]._wasSelected == null) {}
                    if (options[i]._wasSelected != options[i].selected) {
                        ...
                        if (options[i].selected) {
                            this.record("addSelection", this.locatorBuilders.buildAll(event.target), value, baseURI);
                        } else {
                            this.record("removeSelection", this.locatorBuilders.buildAll(event.target), value, baseURI);
                        }
                        options[i]._wasSelected = options[i].selected;
                    }
                }
